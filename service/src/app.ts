...
export function createApp(): Express {
  ...
  app.get(`${prefix}/health`, (_request, response) => {
    response.json({ ok: true, service: 'yuvomi-daybook', version: '0.1.0' });
  });
...
function validateEntryInput(value: unknown): {
  ...
  const occurred = new Date(String(record.occurred_at || ''));
  if (!title) throw new Error('Titel fehlt.');
  if (Number.isNaN(occurred.getTime())) throw new Error('Zeitpunkt ist ungültig.');
